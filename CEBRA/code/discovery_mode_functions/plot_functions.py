...
def plot_decoding_accuracy(ax, arglist, decoding_accuracy_train, decoding_accuracy_test, mice):
    """
    Plot the decoding accuracy.
    Parameters:
        - ax: matplotlib.axes.Axes
        - decoding_accuracy: ndarray, shape (n_samples, n_timepoints).
    Returns:
        - ax: matplotlib.axes.Axes
    """
    ...
    mean_decoding_accuracy_train = np.mean(decoding_accuracy_train, axis=0)
    sem_decoding_accuracy_train = np.std(decoding_accuracy_train, axis=0) / np.sqrt(decoding_accuracy_train.shape[0])

    mean_decoding_accuracy_test = np.mean(decoding_accuracy_test, axis=0)
    sem_decoding_accuracy_test = np.std(decoding_accuracy_test, axis=0) / np.sqrt(decoding_accuracy_test.shape[0])
    if mice == 'control':
        ax.plot(mean_decoding_accuracy_train, linewidth=1, color='navy', label = 'Train')
        ax.fill_between(np.arange(len(mean_decoding_accuracy_train)), mean_decoding_accuracy_train - sem_decoding_accuracy_train, mean_decoding_accuracy_train + sem_decoding_accuracy_train, color='navy', alpha=0.25)

        ax.plot(mean_decoding_accuracy_test, linewidth=1, color='black', label = 'Test')
        ax.fill_between(np.arange(len(mean_decoding_accuracy_test)), mean_decoding_accuracy_test - sem_decoding_accuracy_test, mean_decoding_accuracy_test + sem_decoding_accuracy_test, color='gray', alpha=0.25)
    else:
        ax.plot(mean_decoding_accuracy_train, linewidth=1, color='navy', label='Train')
        ax.fill_between(np.arange(len(mean_decoding_accuracy_train)), mean_decoding_accuracy_train - sem_decoding_accuracy_train, mean_decoding_accuracy_train + sem_decoding_accuracy_train, color='navy', alpha=0.25)

        ax.plot(mean_decoding_accuracy_test, linewidth=1, color=(0.64, 0.08, 0.18), label='Test')
        ax.fill_between(np.arange(len(mean_decoding_accuracy_test)), mean_decoding_accuracy_test - sem_decoding_accuracy_test, mean_decoding_accuracy_test + sem_decoding_accuracy_test, color=(0.64, 0.08, 0.18), alpha=0.25)

    ax.set_xticks([distractor['early'], distractor['middle'], distractor['late']])
    ax.set_xticklabels([-3, -2, -1], fontsize=14)
    ax.set_xlim([0, len(mean_decoding_accuracy_train) - 1])
    ax.set_ylim([50, 100])
    ax.set_yticks([50, 75, 100])
    ax.set_yticklabels([50, 75, 100], fontsize=14)
