...
Please note that if models are trained from scratch, the generated results may not be identical to Figure 3 in the paper due to the inherent randomness of CEBRA.
...
4. **Run discovery mode code**
   ...
   - Run the script:
     ```bash
     python demo_5_discovery_mode.py --root_path /home/chunyue/Documents/Zenodo_APP/CEBRA/ --data_path /home/chunyue/Documents/Zenodo_APP/CEBRA/Data --model_save_path /home/chunyue/Documents/Zenodo_APP/CEBRA/discovery_mode_models/ --device cuda:0 --model_trained True
     ```

     **Notes**:
     - Set the variable **`model_trained`** to `False` when running the `demo_5` file for the first time.
...
- **`--root_path`**  
  The path leads to the parent folder containing the `README.md` file and the `code` folder.
...
Other arguments are defined in the `default_ops.py` file located in the `choice_mode_functions` , `stimulus_mode_functions` or `discovery_mode_functions` folder.
